...
3. The script opens a visible browser and prints `registerTool: true`, `status: "registered"`, and `registeredTools: 11`. A browser without WebMCP exits with a clear setup error instead of claiming success.
...
This script verifies registration only. A registered tool count is not evidence of successful HTTP publication. Use the [local backend runbook](../../docs/operations/local-backend.md) to start the HTTP application, then point `WEBMCP_DEMO_URL` at it. The [native evidence record](../../docs/operations/native-webmcp-evidence.md) distinguishes actual native calls from HTTP BDD and unit tests, and explicitly records the authenticated native gate that remains pending.
