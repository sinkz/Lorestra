...
No modo HTTP essas ferramentas usam D1/R2 persistentes e a sessão autenticada do navegador. Merge exige versões compatíveis, aprovação válida e checks do servidor. O fluxo nativo do agente no navegador tem duas fases de propósito: a primeira chamada de merge retorna um resultado limitado `confirmation_required` e abre uma autorização visível para a proposta aprovada e o hash exatos; aceitar o diálogo não publica. Depois da decisão da pessoa revisora, repita explicitamente o payload idêntico com a chave de idempotência original. Não há nova tentativa automática nem polling apertado. Recusa, cancelamento, expiração, conteúdo obsoleto, descarte da sessão e divergência de payload/chave falham de forma segura. Uma resposta interrompida não autoriza criar outra operação: preserve a chave original e leia a proposta/histórico se a escrita protegida puder ter sido enviada.
